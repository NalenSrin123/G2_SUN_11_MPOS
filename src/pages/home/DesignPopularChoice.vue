...
const menuItems = ref([
  ...
  },
  {
    ...
    price: '$24.00',
    image: 'https://images.unsplash.com/photo-1532636875304-0c8fe119ff9e?auto=format&fit=crop&w=500&q=80',
    featured: false
  },
  {
    id: 5,
    ...
    category: 'Starters',
    tag: 'Vegetarian',
    description: 'Blistered tomatoes, aged balsamic glaze, and fresh basil oil.',
    price: '$19.00',
  ...
  },
  {
    ...
    category: 'Desserts',
    tag: 'Chef Choice',
    description: 'Caramelized puff pastry, Madagascar vanilla bean pastry cream.',
    price: '$16.00',
...
const filteredItems = computed(() => {
  return menuItems.value.filter(item => {
    ...
    const matchesSearch = item.name.toLowerCase().includes(searchQueries.value.toLowerCase()) || 
                          item.description.toLowerCase().includes(searchQueries.value.toLowerCase())
...
<template>
  <div class="min-h-screen bg-white pb-8 font-sans text-slate-900 selection:bg-blue-100">
    
    <header class="sticky top-0 z-50 bg-white shadow-sm transition-shadow duration-300 lg:bg-white/90 lg:backdrop-blur-md">
      <div class="mx-auto flex max-w-6xl items-center justify-between gap-6 px-4 py-4 lg:px-8 lg:py-6">
        
        <h1 class="text-2xl font-bold text-blue-600 lg:text-3xl">LuxeDine</h1>
        
        <div class="hidden flex-1 justify-center max-w-md lg:flex">
          <div class="flex w-full items-center rounded-xl bg-slate-50 px-4 py-2.5 border border-slate-100 focus-within:bg-white focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100 transition-all duration-200">
        ...
        <div class="flex items-center gap-4 lg:gap-5">
          <button 
            ...
            :class="isMobileSearchOpen ? 'text-blue-600 hover:text-red-600' : 'text-blue-600 hover:text-blue-700'"
            :aria-label="isMobileSearchOpen ? 'Close search' : 'Open search'"
          >
            <i :class="isMobileSearchOpen ? 'fas fa-times' : 'fas fa-search'"></i>
          </button>
          
          <button class="relative text-xl text-blue-600 transition duration-200 active:scale-95 hover:text-blue-700" aria-label="Notifications">
      ...
      >
        <div class="flex w-full items-center rounded-xl bg-slate-50 px-4 py-2.5 border border-slate-200 focus-within:bg-white focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100">
          <i class="fas fa-search mr-3 text-slate-400"></i>
          <input 
            ...
            class="w-full text-sm text-slate-800 placeholder-slate-400 outline-none bg-transparent"
          />
          <button v-if="searchQueries" @click="searchQueries = ''" class="text-slate-400 hover:text-slate-600 ml-2">
        ...
        <div v-else class="grid grid-cols-2 gap-4 lg:grid-cols-3 lg:gap-6">
          <div 
            ...
            :class="[
              ...
              'h-[180px] lg:h-[260px] lg:cursor-pointer lg:hover:-translate-y-1 lg:hover:shadow-blue-100 lg:hover:shadow-xl',
              item.featured && activeCategory === 'All' ? 'col-span-2 h-[220px] lg:col-span-3 lg:h-[340px]' : ''
            ]"
            :style="{ backgroundImage: `linear-gradient(to top, rgba(15,23,42,0.95) 10%, rgba(15,23,42,0.2) 60%), url(${item.image})` }"
          >
            <div class="w-full p-4 text-white">
              ...
              <div class="flex flex-col">
                <span class="text-[10px] uppercase font-bold tracking-widest text-blue-300 opacity-90 mb-0.5" v-if="activeCategory === 'All'">
                  {{ item.category }}
                </span>
                <h3 class="font-semibold text-white text-base lg:text-xl">{{ item.name }}</h3>
                <p class="text-xs opacity-85 mt-0.5 mb-2 lg:text-sm line-clamp-1 lg:line-clamp-none">{{ item.description }}</p>
                <span class="font-bold text-sm lg:text-base">{{ item.price }}</span>
              </div>
            </div>
          </div>
        </div>
